...
export default class App extends Component {
  ...
  login = (userLogin) => {
    this.apiService.login(userLogin).then((userInfo) => {
  ...
  render() {
    if (this.state.loginState !== LOGGED_IN) {
      return (
        <LoginScreen login={this.login} loginState={this.state.loginState} />
      );
    }

    return (
      <div>
        <NavMenu username={this.state.userInfo.username} logout={this.logout} />
        <Container>
          <Route exact path="/">
